...
public class AddMoviees extends AddMedia {
	

	GridPane moviees = new GridPane();

	 AddMoviees(MediaRental m, File moviesInfo, File albumInfo, File gameInfo) {

		super(m, moviesInfo, albumInfo, gameInfo);
		this.setTitle("Add Movies");
		this.setScene(new Scene(moviees, 600, 400));
		...
		enterMr.setDisable(true);
		enterCopies.setOnKeyTyped(t -> {
			enterMr.setDisable(false);
		});
		...
		HBox addButton = new HBox(addM, addA);
		...
		addM.setOnAction(y -> {
			if ((enterId.getText() != null && !enterId.getText().isEmpty())) {
				...
				m.addMovie(enterId.getText(), enternameM.getText(), Integer.parseInt(enterCopies.getText()),
						enterMr.getText());
